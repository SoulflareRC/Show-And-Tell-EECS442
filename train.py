...
from utils import train_model, evaluate_model, save_checkpoint, restore_checkpoint, eval, eval2
if __name__ == "__main__":
    ...
    config_name = f"dropout_{feature_extractor}_{feature_dim}_emb_{emb_dim}_hidden_{hidden_size}_Adam_{lr}"

    # config_name = "test"

    '''Dataset'''
    train_ds = Flickr8kDataset2(data_dir,split="train",feature_extractor=feature_extractor)
    test_ds = Flickr8kDataset2(data_dir,split="test",feature_extractor=feature_extractor)
    val_ds = Flickr8kDataset2(data_dir,split="validation",feature_extractor=feature_extractor)
    ...
    '''Setup'''
    model = NIC(vocab_size=vocab_size,embed_dim=emb_dim,hidden_size=hidden_size,feature_dim=feature_dim)
    ...
    model = model.eval()
    scores = eval2(test_ds,train_ds.tokenizer,model)
